...
import DetailGarage from 'views/Admin/Settings/Account/Garage/Detail'
import useGarage from 'data/useGarage'

interface GarageProps {
  defaultPage: number
}

function Garage(props: GarageProps) {
  const defaultPage = get(props, 'defaultPage', 1)
  const defaultPageSize = 10
  const baseUrlPage = `/admin/settings/account/garage`
  ...
  const { data, refetch, helpers, isLoading: queryLoading } = queryGarage
  ...
  const multipleDelete = useMutation((listChecked: string | string[]) =>
    ApiCall.Garage.multipleForceDelete({ ids: listChecked }),
  )
    ...
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} style={{ textAlign: 'right' }}>
          <MyPagination {...Paginations.getPaginationProps(queryGarage)} />
        </Col>
      </Row>
      ...
      <Modal
        ...
        {...stateToggle.state}
      >
        <DetailGarage data={stateToggle.state.data} />
      </Modal>
    </Space>
  )
}
