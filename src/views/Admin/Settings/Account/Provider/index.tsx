...
import DetailProvider from 'views/Admin/Settings/Account/Provider/Detail'
...
function Provider(props: ProviderProps) {
  ...
  const baseUrlPage = `/admin/settings/account/provider`
  ...
  const queryProvider = useProvider({
    query: {
      defaultValue: {
  ...
  const { data, refetch, helpers, isLoading: queryLoading } = queryProvider
  ...
  const multipleDelete = useMutation((listChecked: string | string[]) =>
    ApiCall.Provider.multipleForceDelete({ ids: listChecked }),
  )
    ...
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} style={{ textAlign: 'right' }}>
          <MyPagination {...Paginations.getPaginationProps(queryProvider)} />
        </Col>
      </Row>
      ...
      <Modal
        ...
        {...stateToggle.state}
      >
        <DetailProvider data={stateToggle.state.data} />
      </Modal>
    </Space>
  )
}
