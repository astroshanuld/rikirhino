...
import useInsuranceProduct from 'data/useInsuranceProduct'
import DetailInsuranceProduct from 'views/Admin/Product/Insurance/Detail'

function InsuranceProduct(props: any) {
  const defaultPage = get(props, 'router.query.page', 1)
  const defaultPageSize = 10
  const baseUrlPage = `/admin/product/insurance`
  ...
  const stateToggle = useToggle({
    ...
    initialState: {
      ...
      data: {
        ...
        WorkshopProductId: null,
        WorkshopProduct: null,
        createdAt: null,
        updatedAt: null,
        deletedAt: null,
      },
  ...
  const queryInsuranceProduct = useInsuranceProduct({
    query: {
      defaultValue: {
  ...
  const multipleDelete = useMutation((listChecked: string | string[]) =>
    ApiCall.InsuranceProduct.multipleForceDelete({ ids: listChecked }),
  )
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
      <div id={cssContent.customPageHeader}>
        <PageHeader title="Product" subTitle={'Insurance'} />
      </div>
      ...
      <div className={cssContent.contentPage}>
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} style={{ textAlign: 'right' }}>
            <MyPagination
              {...Paginations.getPaginationProps(queryInsuranceProduct)}
            />
          </Col>
        </Row>
        ...
        <Modal
          ...
          {...stateToggle.state}
        >
          <DetailInsuranceProduct data={stateToggle.state.data} />
        </Modal>
      </div>
    </Space>
  )
